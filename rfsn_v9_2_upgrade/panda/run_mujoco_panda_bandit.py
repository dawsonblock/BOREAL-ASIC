...
Usage:
    ...
    # With visualization
    python panda/run_mujoco_panda_bandit.py --xml path/to/panda.xml --episodes 10 --render
...
from vw_bandit import VWConfig, VWContextualBandit, RewardNormalizer, RewardDecayScheduler
...
from panda.panda_context import build_context, context_to_feature_vector
from panda.panda_reward import compute_reward, PandaRewardConfig, is_success
...
from panda.mujoco_panda_obs import get_single_obs
...
class PandaBanditRunner:
    ...
    def run_episode(self, targets: PrimitiveTargets, viewer=None) -> dict:
        ...
        for step in range(self.episode_cfg.max_steps):
            ...
            # Execute primitive
            info = self.prims.step(action, targets)
            
            # Step physics
            for _ in range(self.episode_cfg.steps_per_action):
            ...
            slip = False  # Implement slip detection if needed
...
def main():
    ...
    parser.add_argument(
        "--ee-site", type=str, default="attachment_site",
        help="End-effector site name in XML"
    )
    parser.add_argument(
        "--obj-body", type=str, default="target_object",
        help="Object body name in XML"
    )
    parser.add_argument(
        "--table-body", type=str, default="table",
