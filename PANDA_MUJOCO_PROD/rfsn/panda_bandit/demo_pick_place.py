...
# This demo uses the integrated rfsn package structure.  Ensure that
# MuJoCo is installed; otherwise print a helpful error and exit.
try:
    import mujoco
except ImportError:
    print("ERROR: MuJoCo is required for this demo. Please install mujoco via `pip install mujoco`.\n")
    sys.exit(1)
...
def run_demo():
    print("Loading scene...")
    # Load the pick-and-place scene relative to this script.  The
    # panda_pick_place_scene.xml is located alongside this file in the
    # rfsn.panda_bandit package.
    scene_path = Path(__file__).resolve().parent / "panda_pick_place_scene.xml"
