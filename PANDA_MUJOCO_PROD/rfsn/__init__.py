...
# Domain randomization utilities depend on MuJoCo.  If MuJoCo is not
# installed, expose stubs that raise informative ImportError when used.
try:
    ...
    if _e.name == "mujoco":
        def _requires_mujoco_domain(*args, **kwargs):  # type: ignore
            """Stub that raises when MuJoCo-dependent domain randomization is used."""
            raise ImportError(
                "MuJoCo must be installed to use domain randomization components. "
                "Install MuJoCo (pip install mujoco) before using these APIs."
            )
        DomainRandomizer = DomainRandomizationConfig = RandomizationState = get_preset_config = PRESET_CONFIGS = _requires_mujoco_domain  # type: ignore
    else:
        raise
