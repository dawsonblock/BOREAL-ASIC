...
class PandaPickEnv:
    ...
    def __init__(self, model_path="panda_table_cube.xml", render_mode=None):
        ...
        # Gripper Geoms
        self.gripper_geoms = [
            mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "panda_finger_left_geom"),
            mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "panda_finger_right_geom"),
            mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, "panda_hand_geom")
        ]
    ...
    def _compute_reward(self, obs, action):
        ...
        # 2. Contact Bonus
        # Check if gripper fingers are touching cube geom
        has_contact = False
        for i in range(self.data.ncon):
            con = self.data.contact[i]
            g1, g2 = con.geom1, con.geom2
            if (g1 == self.cube_geom and g2 in self.gripper_geoms) or \
               (g2 == self.cube_geom and g1 in self.gripper_geoms):
                has_contact = True
                break
    ...
    def solve_ik(self, target_pos):
        ...
        for _ in range(50):
            ...
            jacr = np.zeros((3, self.ik_model.nv))
            mujoco.mj_jac(self.ik_model, self.ik_data, jacp, jacr, curr_pos, self.hand_id)
