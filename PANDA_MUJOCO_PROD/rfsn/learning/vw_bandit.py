...
try:
    ...
    try:
        import vowpal_wabbit_next as vw_next
        VW_V = "next"
        ...
        class pyvw:
            class vw:
                def __init__(self, args):
                    # args is a string, split for next
                    if isinstance(args, str):
                        args = args.split()
                    self.workspace = vw_next.Workspace(args)
                    self.parser = vw_next.TextFormatParser(self.workspace)
                
                def predict(self, ec):
                    if not isinstance(ec, str):
                        # Handle case where ec might be an example already?
                        # But existing code passes string.
                        return 0
                    ex = self.parser.parse_line(ec)
                    return self.workspace.predict_one(ex)
                
                def learn(self, ec):
                    ex = self.parser.parse_line(ec)
                    self.workspace.learn_one(ex)
                
                def save(self, filename):
                    with open(filename, "wb") as f:
                        f.write(self.workspace.serialize())
        ...
        VW_V = "none"
        # We define a dummy pyvw to avoid NameErrors at module level, 
        # though instantiation will fail if used.
        class pyvw:
             class vw:
                 def __init__(self, *args, **kwargs):
                     raise ImportError("Vowpal Wabbit not installed")
...
class VWConfig:
    ...
    def to_vw_args(self) -> List[str]:
        ...
        # Use space-separated format for VW 9.x compatibility
        args = [
            f"--learning_rate", f"{self.learning_rate}",
            f"--power_t", f"{self.power_t}",
            f"--initial_t", f"{self.initial_t}",
            f"-b", f"{self.bits}",
            f"--l2", f"{self.l2_regularization}",
        ]
        
        if self.l1_regularization > 0:
            args.extend([f"--l1", f"{self.l1_regularization}"])
        
        if self.quiet:
            args.append("--quiet")
        
        args.extend([f"--random_seed", f"{self.random_seed}"])
...
class VWContextualBandit:
    ...
    def __init__(self, config: VWConfig):
        ...
        # Initialize VW workspace
        # Note: Using --cb (not --cb_explore_adf) for VW 9.x compatibility
        vw_arg_str = (
            f"--cb {config.n_actions} "
    ...
    def select_action(
        ...
        """
        # Build simple context string for --cb mode
        vw_input = self._context_to_vw_format(context)
        
        # Get VW's prediction (action index for --cb mode)
        prediction = self.vw.predict(vw_input)
        vw_action = int(prediction) - 1  # VW uses 1-indexed actions
        vw_action = max(0, min(vw_action, self.config.n_actions - 1))
    ...
    def update(
        ...
        if action_prob is None:
            action_prob = 1.0 / self.config.n_actions
        
        # VW --cb format: "action:cost:probability |features"
        # Note: VW uses 1-indexed actions, and cost = -reward (VW minimizes cost)
        vw_action = action + 1  # Convert to 1-indexed
        cost = -reward  # VW minimizes cost, so negate reward
        
        vw_input = f"{vw_action}:{cost}:{action_prob:.6f} {self._context_to_vw_format(context)}"
        self.vw.learn(vw_input)
